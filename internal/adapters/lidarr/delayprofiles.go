...
// because Lidarr creates a default delay profile that we may need to modify
func (a *Adapter) getManagedDelayProfiles(ctx context.Context, c *httpClient) ([]irv1.DelayProfileIR, error) {
	...
	if err := c.get(ctx, "/api/v1/delayprofile", &profiles); err != nil {
...
// delayProfileToIR converts a Lidarr delay profile to IR
...
// irToDelayProfile converts IR to Lidarr delay profile resource
...
func (a *Adapter) createDelayProfile(ctx context.Context, c *httpClient, ir *irv1.DelayProfileIR, tagID int) error {
	...
	return c.post(ctx, "/api/v1/delayprofile", profile, nil)
}
...
func (a *Adapter) updateDelayProfile(ctx context.Context, c *httpClient, ir *irv1.DelayProfileIR, tagID int) error {
	...
	return c.put(ctx, fmt.Sprintf("/api/v1/delayprofile/%d", ir.ID), profile, nil)
}
...
func (a *Adapter) deleteDelayProfile(ctx context.Context, c *httpClient, id int) error {
	return c.delete(ctx, fmt.Sprintf("/api/v1/delayprofile/%d", id))
}
