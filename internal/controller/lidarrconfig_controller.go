...
func (r *LidarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.ensureInitialized()
	return r.generic.FetchAndReconcile(ctx, req, LidarrConfigFetcher{})
}

// ensureInitialized initializes the generic reconciler if not already done.
// NOTE: Lidarr uses ProwlarrCoordinatorReconciler for Prowlarr registration.
// The Coordinator has a holistic view and can detect Push/Pull conflicts.
func (r *LidarrConfigReconciler) ensureInitialized() {
	if r.generic != nil {
		return
	}
	if r.Compiler == nil {
	...
	r.generic = &GenericArrReconciler{
		...
		CompileConfig: func(ctx context.Context, c *compiler.Compiler, config ArrConfigObject, secrets map[string]string, caps *adapters.Capabilities) (*irv1.IR, error) {
			lidarrConfig := config.(*LidarrConfigAdapter).LidarrConfig
			return c.CompileLidarrConfig(ctx, lidarrConfig, secrets, caps)
		},
	}
}
