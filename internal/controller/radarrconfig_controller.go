...
const radarrFinalizer = "radarrconfig.arr.rinzler.cloud/finalizer"
...
func (r *RadarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the RadarrConfig
	config := &arrv1alpha1.RadarrConfig{}
	if err := r.Get(ctx, req.NamespacedName, config); err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("RadarrConfig resource not found, ignoring")
			return ctrl.Result{}, nil
		}
		log.Error(err, "Failed to get RadarrConfig")
	...
	// Ensure finalizer
	if !controllerutil.ContainsFinalizer(config, radarrFinalizer) {
		controllerutil.AddFinalizer(config, radarrFinalizer)
...
func (r *RadarrConfigReconciler) reconcileNormal(ctx context.Context, config *arrv1alpha1.RadarrConfig) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Reconciling RadarrConfig", "name", config.Name)

	statusWrapper := &RadarrStatusWrapper{Status: &config.Status}
	...
	adapter, ok := adapters.Get(adapters.AppRadarr)
	if !ok {
		r.Helper.SetCondition(statusWrapper, config.Generation, ConditionTypeReady, metav1.ConditionFalse, "AdapterNotFound", "radarr adapter not registered")
		if statusErr := r.Status().Update(ctx, config); statusErr != nil {
			log.Error(statusErr, "Failed to update status")
		}
	...
	// Compile CRD to IR
	desiredIR, err := r.Compiler.CompileRadarrConfig(ctx, config, resolvedSecrets, caps)
	if err != nil {
		log.Error(err, "Failed to compile RadarrConfig to IR")
	...
	_, err = r.Helper.ReconcileConfig(ctx, adapters.AppRadarr, connIR, desiredIR, statusWrapper, config.Generation)
	if err != nil {
		if statusErr := r.Status().Update(ctx, config); statusErr != nil {
			log.Error(statusErr, "Failed to update status")
		}
	...
	_, err = r.Helper.ApplyDirectConfig(ctx, adapters.AppRadarr, connIR, desiredIR, statusWrapper, config.Generation)
	if err != nil {
	...
	if config.Spec.Indexers != nil && config.Spec.Indexers.ProwlarrRef != nil {
		reg := ProwlarrAutoRegistration{
			...
			AppType:     adapters.AppRadarr,
			AppName:     fmt.Sprintf("nebularr-%s-%s", adapters.AppRadarr, config.Name),
...
// reconcileDelete handles deletion of the RadarrConfig
func (r *RadarrConfigReconciler) reconcileDelete(ctx context.Context, config *arrv1alpha1.RadarrConfig) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Handling deletion of RadarrConfig", "name", config.Name)
	...
	if err != nil {
		...
		if err := r.Helper.CleanupManagedResources(ctx, adapters.AppRadarr, connIR); err != nil {
			log.Error(err, "Failed to cleanup managed resources")
		}
	}

	// Remove finalizer
	controllerutil.RemoveFinalizer(config, radarrFinalizer)
	if err := r.Update(ctx, config); err != nil {
		return ctrl.Result{}, err
	}

	log.Info("Successfully deleted RadarrConfig", "name", config.Name)
	return ctrl.Result{}, nil
}
