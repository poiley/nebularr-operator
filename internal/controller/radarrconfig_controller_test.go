...
var _ = Describe("RadarrConfig Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind RadarrConfig")
			err := k8sClient.Get(ctx, typeNamespacedName, radarrconfig)
			if err != nil && errors.IsNotFound(err) {
				resource := &arrv1alpha1.RadarrConfig{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance RadarrConfig")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &RadarrConfigReconciler{
				Client: k8sClient,
