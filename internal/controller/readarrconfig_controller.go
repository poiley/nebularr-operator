...
func (r *ReadarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.ensureInitialized()
	return r.generic.FetchAndReconcile(ctx, req, ReadarrConfigFetcher{})
}

// ensureInitialized initializes the generic reconciler if not already done.
func (r *ReadarrConfigReconciler) ensureInitialized() {
	if r.generic != nil {
		return
	}
	if r.Compiler == nil {
	...
	r.generic = &GenericArrReconciler{
		...
		CompileConfig: func(ctx context.Context, c *compiler.Compiler, config ArrConfigObject, secrets map[string]string, caps *adapters.Capabilities) (*irv1.IR, error) {
			readarrConfig := config.(*ReadarrConfigAdapter).ReadarrConfig
			return c.CompileReadarrConfig(ctx, readarrConfig, secrets, caps)
		},
	}
}
