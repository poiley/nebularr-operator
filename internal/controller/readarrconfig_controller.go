...
import (
	...
	_ "github.com/poiley/nebularr-operator/internal/adapters/readarr" // Register readarr adapter
	"github.com/poiley/nebularr-operator/internal/compiler"
	irv1 "github.com/poiley/nebularr-operator/internal/ir/v1"
)

const readarrFinalizer = "readarrconfig.arr.rinzler.cloud/finalizer"

// ReadarrConfigReconciler reconciles a ReadarrConfig object
...
// +kubebuilder:rbac:groups=arr.rinzler.cloud,resources=readarrconfigs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=arr.rinzler.cloud,resources=readarrconfigs/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=arr.rinzler.cloud,resources=readarrconfigs/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop
func (r *ReadarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx)

	// Fetch the ReadarrConfig
	config := &arrv1alpha1.ReadarrConfig{}
	if err := r.Get(ctx, req.NamespacedName, config); err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("ReadarrConfig resource not found, ignoring")
			return ctrl.Result{}, nil
		}
		log.Error(err, "Failed to get ReadarrConfig")
	...
	// Ensure finalizer
	if !controllerutil.ContainsFinalizer(config, readarrFinalizer) {
		controllerutil.AddFinalizer(config, readarrFinalizer)
...
func (r *ReadarrConfigReconciler) reconcileNormal(ctx context.Context, config *arrv1alpha1.ReadarrConfig) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Reconciling ReadarrConfig", "name", config.Name)

	statusWrapper := &ReadarrStatusWrapper{Status: &config.Status}
	...
	adapter, ok := adapters.Get(adapters.AppReadarr)
	if !ok {
		r.Helper.SetCondition(statusWrapper, config.Generation, ConditionTypeReady, metav1.ConditionFalse, "AdapterNotFound", "readarr adapter not registered")
		if statusErr := r.Status().Update(ctx, config); statusErr != nil {
			log.Error(statusErr, "Failed to update status")
		}
	...
	// Compile CRD to IR
	desiredIR, err := r.Compiler.CompileReadarrConfig(ctx, config, resolvedSecrets, caps)
	if err != nil {
		log.Error(err, "Failed to compile ReadarrConfig to IR")
	...
	_, err = r.Helper.ReconcileConfig(ctx, adapters.AppReadarr, connIR, desiredIR, statusWrapper, config.Generation)
	if err != nil {
		if statusErr := r.Status().Update(ctx, config); statusErr != nil {
			log.Error(statusErr, "Failed to update status")
		}
	...
	_, err = r.Helper.ApplyDirectConfig(ctx, adapters.AppReadarr, connIR, desiredIR, statusWrapper, config.Generation)
	if err != nil {
	...
	if config.Spec.Indexers != nil && config.Spec.Indexers.ProwlarrRef != nil {
		reg := ProwlarrAutoRegistration{
			...
			AppType:     adapters.AppReadarr,
			AppName:     fmt.Sprintf("nebularr-%s-%s", adapters.AppReadarr, config.Name),
	...
	healthStatus := r.Helper.CheckAndReportHealth(ctx, adapters.AppReadarr, connIR, config, r.Recorder)
	if healthStatus != nil {
...
// reconcileDelete handles deletion of the ReadarrConfig
func (r *ReadarrConfigReconciler) reconcileDelete(ctx context.Context, config *arrv1alpha1.ReadarrConfig) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Handling deletion of ReadarrConfig", "name", config.Name)
	...
	if err != nil {
		...
		if err := r.Helper.CleanupManagedResources(ctx, adapters.AppReadarr, connIR); err != nil {
			log.Error(err, "Failed to cleanup managed resources")
		}
	}

	// Remove finalizer
	controllerutil.RemoveFinalizer(config, readarrFinalizer)
	if err := r.Update(ctx, config); err != nil {
		return ctrl.Result{}, err
	}

	log.Info("Successfully deleted ReadarrConfig", "name", config.Name)
	return ctrl.Result{}, nil
}
