...
var _ = Describe("ReadarrConfig Controller", func() {
	const (
	...
	Context("When reconciling a resource", func() {
		const (
			resourceName = "test-readarr"
			namespace    = "default"
			secretName   = "readarr-credentials"
		)

		var (
			ctx               context.Context
			typeNamespaceName types.NamespacedName
			readarrConfig     *arrv1alpha1.ReadarrConfig
			apiKeySecret      *corev1.Secret
			mockAdapter       *mock.Adapter
			reconciler        *ReadarrConfigReconciler
		)
		...
		BeforeEach(func() {
			...
			// Setup mock adapter
			mockAdapter = mock.NewAdapter("readarr")
			...
			// Create ReadarrConfig resource
			readarrConfig = &arrv1alpha1.ReadarrConfig{
				ObjectMeta: metav1.ObjectMeta{
					Name:      resourceName,
					Namespace: namespace,
				},
				Spec: arrv1alpha1.ReadarrConfigSpec{
					Connection: arrv1alpha1.ConnectionSpec{
						URL: "http://readarr.example.com:8787",
		...
		AfterEach(func() {
			...
			// Clean up ReadarrConfig
			resource := &arrv1alpha1.ReadarrConfig{}
		...
		It("should successfully reconcile and set Ready condition", func() {
			By("Creating the ReadarrConfig resource")
			Expect(k8sClient.Create(ctx, readarrConfig)).To(Succeed())
			...
			updatedConfig := &arrv1alpha1.ReadarrConfig{}
		...
		It("should set Connected status and version from adapter", func() {
			...
			mockAdapter.WithVersion("0.3.0.1234")
			...
			Expect(updatedConfig.Status.ServiceVersion).To(Equal("0.3.0.1234"))
		})
		...
		It("should set Ready=False when API key secret is missing", func() {
			By("Creating ReadarrConfig with non-existent secret")
			readarrConfig.Spec.Connection.APIKeySecretRef.Name = "non-existent-secret"
			Expect(k8sClient.Create(ctx, readarrConfig)).To(Succeed())
		...
		It("should requeue with custom interval", func() {
			By("Creating ReadarrConfig with custom reconciliation interval")
			interval := metav1.Duration{Duration: 10 * time.Minute}
			readarrConfig.Spec.Reconciliation = &arrv1alpha1.ReconciliationSpec{
				Interval: &interval,
			}
			Expect(k8sClient.Create(ctx, readarrConfig)).To(Succeed())
		...
		It("should add and remove finalizer", func() {
			...
			updatedConfig := &arrv1alpha1.ReadarrConfig{}
			Expect(k8sClient.Get(ctx, typeNamespaceName, updatedConfig)).To(Succeed())
			Expect(updatedConfig.Finalizers).To(ContainElement("readarrconfig.arr.rinzler.cloud/finalizer"))

			By("Deleting the resource")
			Expect(k8sClient.Delete(ctx, updatedConfig)).To(Succeed())
			...
			By("Checking resource was deleted")
		...
		It("should track ManagedResources in status", func() {
			...
			mockAdapter.WithChanges(&adapters.ChangeSet{
				Creates: []adapters.Change{
					{ResourceType: adapters.ResourceQualityProfile, Name: "Standard"},
				},
			})
		...
		It("should respect reconciliation suspend flag", func() {
			By("Creating ReadarrConfig with suspend=true")
			readarrConfig.Spec.Reconciliation = &arrv1alpha1.ReconciliationSpec{
				Suspend: true,
			}
			Expect(k8sClient.Create(ctx, readarrConfig)).To(Succeed())
