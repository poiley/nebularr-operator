...
func (r *SonarrConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.ensureInitialized()
	return r.generic.FetchAndReconcile(ctx, req, SonarrConfigFetcher{})
}

// ensureInitialized initializes the generic reconciler if not already done.
// This supports both SetupWithManager and direct instantiation (e.g., tests).
func (r *SonarrConfigReconciler) ensureInitialized() {
	if r.generic != nil {
		return
	}
	if r.Compiler == nil {
	...
	r.generic = &GenericArrReconciler{
		...
		CompileConfig: func(ctx context.Context, c *compiler.Compiler, config ArrConfigObject, secrets map[string]string, caps *adapters.Capabilities) (*irv1.IR, error) {
			sonarrConfig := config.(*SonarrConfigAdapter).SonarrConfig
			return c.CompileSonarrConfig(ctx, sonarrConfig, secrets, caps)
		},
	}
}
