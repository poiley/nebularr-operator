...
var _ = Describe("SonarrConfig Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind SonarrConfig")
			err := k8sClient.Get(ctx, typeNamespacedName, sonarrconfig)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance SonarrConfig")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &SonarrConfigReconciler{
				Client: k8sClient,
